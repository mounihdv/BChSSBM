...
st.set_page_config(page_title="BChS Research Dashboard v5", layout="wide")
...
def run_bchs(c, a, p, steps, init_type, pos_count=None):
    ...
    if init_type == "Symmetric (All +1)": m = np.full(c, 0.5)
        ...
        m[:pos_count], m[pos_count:] = 0.5, -0.5
...
if analysis_mode == "Dynamics":
    ...
    with col1:
        ...
        im = ax2.imshow(m_hist.T, aspect='auto', cmap='coolwarm', vmin=-1, vmax=1, interpolation='nearest')
        ticks = range(0, c_slider, 5 if c_slider > 20 else 1)
        ax2.set_yticks(ticks); ax2.set_yticklabels(ticks)
        plt.colorbar(im, label="$m_g$", ax=ax2)
        ax2.set_xlabel("Time"); ax2.set_ylabel("Group Index")
        st.pyplot(fig2)
    ...
    with col2:
        ...
        ax3.plot(ops["polar"], label="Group Polarization $⟨|m_g|⟩$")
...
# --- 4. Phase Space Explorer Mode ---
    ...
    if col_btn1.button("🚀 Calculate"):
        ...
        st.session_state['phase_data'] = {'g_map': g_map, 'p_map': p_map, 'v_map': v_map, 'p_lims': p_range, 'a_lims': a_range, 'c_used': c_slider}
    ...
    if 'phase_data' in st.session_state:
        ...
        # 1. Existence Boundary (Green)
        p_green = np.clip(0.5 * (1 - 1/(2 * np.maximum(lam, 1e-9))), 0, 1)

        # 2. Refined Stability Boundary (Blue Line - Your New Quadratic)
        def get_p_stable_refined(a_v, c_v):
            if a_v <= 1/c_v: return 0.0 
            lam_v = (a_v * c_v - 1) / (c_v - 1)
            # Quadratic: (1+lam)q^2 + (2*lam^2 + lam - 2)q - lam^2 = 0
            coeffs = [(1 + lam_v), (2 * lam_v**2 + lam_v - 2), -(lam_v**2)]
            roots = np.roots(coeffs)
            q_c = np.max(roots[np.isreal(roots)].real)
            return np.clip(0.5 * (1 - q_c / lam_v), 0, 1)

        p_blue = [get_p_stable_refined(v, data['c_used']) for v in a_theory]
